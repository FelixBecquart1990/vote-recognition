...
const imageBucketPrivate = "https://i.imgur.com/";
...
async function loadImage(imagePath) {
  ...
  if (imagePath.startsWith("DG") || imagePath.startsWith("AL")) {
    image.src = `${imageBucketPrivate}${imagePath}`;
  } else {
...
async function testImageAndEstimatePoses(net) {
  ...
  // 5: {position: {…}, part: "leftShoulder", score: 0.1906055063009262}
  // 6: {position: {…}, part: "rightShoulder", score: 0.552233874797821}
  // 7: {position: {…}, part: "leftElbow", score: 0.5563686490058899}
  // 8: {position: {…}, part: "rightElbow", score: 0.8609413504600525}
  // 9: {position: {…}, part: "leftWrist", score: 0.6562609076499939}
  // 10: {position: {…}, part: "rightWrist", score: 0.49961477518081665}

  let vote_yes = 0;
  let vote_no = 0;
  var i = 0;
  console.log(poses);
  while (poses[i] && poses[i].score > 0.1) {
    if (
      poses[i].keypoints[5].position.y > poses[i].keypoints[9].position.y ||
